...
const Navbar = () => {
    ...
    const { coins, isLogged , setIsLogged, watchlist, setWatchlist } = useContext(AppContext);
                ...
                <div className={styles.login} >
                    ...
                    anchor='right' open={isDrawerOpen} onClose={() => setIsDrawerOpen(false)}>
                        <div className={styles.drawer} >
                            ...
                            {docSnapshot?.map(e =>  <div className={styles.navCoins} onClick={() => {navigate(`/coin/${e}`); setIsDrawerOpen(false)}} key={e}>{e}</div>
                            ...
                            {coins?.map(e =>  {
                                if(docSnapshot?.includes(e.id)){
                                    return (
                                        <div className={styles.navSection} key={e.id} onClick={() => {navigate(`/coin/${e.id}`); setIsDrawerOpen(false)}} >
                                            <img className={styles.navIcons} src={e.image} />
